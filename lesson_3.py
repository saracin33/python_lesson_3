...
# очистить текст от знаков препинания и сформировать list со словами:
punctuation = (temp_str.replace('.','').replace(',','').replace('-','').replace('«','').replace('»','').replace('!','').replace('?','').replace('—','').replace(';','').replace(':','').replace('(','').replace(')',''))
words = punctuation.split()

# привести все слова к нижнему регистру:
little_words = list(map(lambda x: x.lower(), words))
print(little_words)

# получить из list пункта 3 dict, ключами которого являются слова, а значениями их количество появлений в тексте:
dict_temp = {}
for i in range(len(little_words)):
    sum = 0
    word = little_words[i]
    for a in range(len(little_words)):
        if word == little_words[a]:
           sum+=1
    dict_temp[word] = sum
print(dict_temp)

# вывести 5 наиболее часто встречающихся слов:
new_list = list(dict_temp.items())
new_list.sort(key = lambda i: i[1],reverse = True)
print(new_list[:5])

# вывести количество разных слов в тексте:
new_set = len(set(new_list))
print(new_set)
